...
pub fn DrawDeadScreen() void {
    ...
    const score_text = std.fmt.bufPrintZ(&main_mod.buf, "Score: {d:.0}", .{main_mod.score}) catch "";
    ...
    const highScore_text = std.fmt.bufPrintZ(&main_mod.buf, "High Score: {d:.0}", .{main_mod.high_score}) catch "";
